...
export const factChecker = task({
  ...
  run: async ({ article }) => {
    ...
    const {
      runs: [extractResult],
    } = await batch.triggerByTaskAndWait([{ task: extractClaims, payload: { article } }])
    ...
    const verified = runs.filter((r): r is typeof r & { ok: true } => r.ok).map((r) => r.output)
...
const results = await myTask.batchTriggerAndWait([{ payload: item1 }, { payload: item2 }])
