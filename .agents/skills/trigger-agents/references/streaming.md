...
export const workerTask = task({
  ...
  run: async ({ item }) => {
    ...
    progressStream.append(JSON.stringify({ item: item.id, status: 'done' }), { target: 'parent' })
...
export const orchestrator = task({
  ...
  run: async ({ items }) => {
    ...
    return workerTask.batchTriggerAndWait(items.map((item) => ({ payload: { item } })))
  },
})
```
...
function TaskProgress({ runId, accessToken }: Props) {
  const { run, streams } = useRealtimeRunWithStreams<typeof processItems, STREAMS>(runId, {
    accessToken,
  })
