...
- Introduces or modifies a domain primitive (Agent, Workflow, Plugin, Tenant, Connection, etc.)
...
- Each bullet should be a **real scenario**, not a category. Not "enables flexibility" but "a tenant creates an agent with no tools, it sits next to live ones in the list, a teammate clicks it expecting chat and gets nothing."
...
Read `docs/vision.md`, an ADR, or `docs/architecture/overview.md` only when it directly grounds a scenario. Not as ritual.
...
When the idea is sharp enough to write down, run `/create-prd` to capture it as a PRD GitHub issue. See `docs/guides/dev-process.md` for the full chain.
